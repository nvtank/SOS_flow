...
3. Trên Dashboard, bấm **Bắt đầu mô phỏng**. Thao tác này reset riêng dữ liệu simulator rồi bắt đầu phát event x1. Demo panel chỉ xuất hiện trong `DEMO_MODE`; API demo yêu cầu `X-Demo-Token`.
...
3. Mở Dashboard, bấm **Bắt đầu mô phỏng**: gọi 112, SMS sai chính tả, Web, Zalo simulator và cán bộ xã lần lượt đi qua API thật. Có thể chọn x2/x5 mà không reset tiến độ, hoặc Pause rồi bấm **Sự kiện kế**. Chỉ rõ badge “Nghi trùng”. Đây đều là simulator, không phải SMS/Zalo/112 production.
4. Chờ autoplay hoàn tất hoặc bấm **Bơm tất cả**. Charts, alert Critical chưa giao đội, mission BLOCKED, NEED_REINFORCEMENT và vòng tròn vùng im lặng cần xác minh tại Trà Linh cập nhật qua API thật.
